...
class ComicPanelView: UIImageView {
    ...
    //MARK: Traslation
    @IBInspectable var moveBy: String = "(0,0)"
    ...
    //MARK: Traslation
    private let parenthesisCharacterSet = CharacterSet(charactersIn: "()")
    private var trimmedMoveBy: String {
        return moveBy.trimmingCharacters(in: parenthesisCharacterSet)
    }
    private var moveXBy: String {
        return trimmedMoveBy.components(separatedBy: ",").first ?? "0"
    }
    private var moveYBy: String {
        return trimmedMoveBy.components(separatedBy: ",").last ?? "0"
    }
    private let traslationStartX: CGFloat = 0
    private let traslationStartY: CGFloat = 0
    private var traslationEndX: CGFloat {
        let moveXByCGFloat = CGFloat(NumberFormatter().number(from: moveXBy)?.doubleValue ?? 0.0)/375
        return max(-1,min(1,moveXByCGFloat))
    }
